...
def main():
    ...
    for i in range(len(clause_set)):
        last_end.append(0)
    keep = True
    model = 0   # 用于转换归并原理的策略，model==0时使用单元优先策略，model==1时使用普遍策略
    while keep:
        last_len = len(clause_set)
        # 采用单元优先策略,先只用单元子句和其他子句进行归结，没有新子句产生时，再使用最一般的归结原则
        for i in range(len(clause_set)):
            if model == 0 and len(clause_set[i]) != 1:
                continue
            if model == 1 and len(clause_set[i]) == 1:
                continue
            if not keep:
                break
            ...
            for j in range(this_start, len(clause_set)):  # clause_set[j]超过一个谓词的取或的子句
                if not keep:
                    break
                if model == 0 and i == j:      # i >= j 在普通形式时使用
                    continue
                if model == 1 and (i >= j or len(clause_set[j])) == 1:  # i >= j 在普通形式时使用
                    continue
                ...
                if is_break(new_clause, clause_set):
                    keep = False
                    break
        if model == 0 and last_len == len(clause_set):     # 如果没有新子句产生，再使用普遍形式
            model = 1
        elif model == 1 and last_len == len(clause_set):     # 仍然没有新的子句产生时，说明无法得到[]，说明要证明的结论错误
            keep = False
            print("Not True")
        else:
            model = 0
...
归结原则：(¬A,C1)∧(A,C2) => (C1,C2)
以上三个测试样例用只用到规则1就能通过(归结规则中特殊的一种情况,单元优先策略)： (A)∧(¬A,C) => C
如果仅使用规则1，未考虑普遍形式的归结原则： (¬A,C1)∧(A,C2) => (C1,C2) 因此对于以下测试样例过不了(是不完备的)
...
但只使用规则一和规则二仍是不完备的，此时可选择单元优先策略：
首先只用单元子句和其他子句进行归结，若没有新子句产生，再使用普遍形式的归结原则进行归结，
若仍没有新子句产生，则要证明的结论的错误的
"""
