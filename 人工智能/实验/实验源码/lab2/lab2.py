...
def exit_yet(new_clause, clause_set, key2):	 # 此处判断为判断是否生成了一个已经存在的子句
    key = key2
    if len(new_clause) == 1:
...
def mgu(clause1, clause2, key1, key2, old_name, new_name, new_clause):  # (¬A,C1)∧(A,C2) => (C1,C2)
    ...
    new_clause.clear()
    the_key = -1
    if num == 1:    # num>1时说明生成的新子句中含有互补的两个谓词，真值恒为1；num=0表示没有互补的两个谓词
...
def main():
    ...
    while keep:
        ...
        # 采用单元优先策略,先只用单元子句和其他子句进行归结，没有新子句产生时，再使用一般的归结原则
        for i in range(len(clause_set)):
            ...
            if not keep:
                break
            this_start = last_end[i]  # 本次while循环的第i子句的开始是上次while循环第i子句的结束
        ...
        if model == 0 and last_len == len(clause_set):     # 如果没有新子句产生，再使用普遍形式
            model = 1
        elif model == 1 and last_len == len(clause_set):     # 仍然没有新的子句产生时，说明无法得到[]，说明无法证明子句集真值为假
            keep = False
            print("Not True")
        else:   # 否则，采取单元优先策略
            model = 0
